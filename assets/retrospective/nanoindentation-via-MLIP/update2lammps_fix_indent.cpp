...
void FixIndent::post_force(int vflag)
{
  ...
  if (istyle == SPHERE) {
    ...
    double angle = 3.1415926535897932384626433832795 / 2;	// angle of the cone
    angle /= 2;
    ...
    double delx,dely,delz,r,dr,fmag,fx,fy,fz, rc, sina, cosa;
    ...
    for (int i = 0; i < nlocal; i++)
      if (mask[i] & groupbit) {
        ...
        sina = sin(angle);
        cosa = cos(angle);
        rc = sqrt(delx*delx + dely*dely);
        if (delz > 0 && rc <= delz*sina/cosa)
          error->all(FLERR,"Some atoms are inside the tip!!!!!!!!");

        if (delz*cosa + rc*sina > 0)
        { 
          r = rc*cosa - delz*sina; 
          dr = r - radius;
          fmag = k*dr*dr;
          fx = fmag*cosa*delx/rc;
          fy = fmag*cosa*dely/rc;
          fz = -fmag*sina;
        }
        else
        {
          r = sqrt(rc*rc + delz*delz);
          dr = r - radius;
          fmag = k*dr*dr;
          fx = delx*fmag/r;
          fy = dely*fmag/r;
